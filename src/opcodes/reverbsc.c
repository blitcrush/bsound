...
static int32_t delay_line_bytes_alloc(SC_REVERB *p, int32_t n)
{
	...
	nBytes = (int32_t)sizeof(reverbsc_delayLine) - (int32_t)sizeof(MYFLT);
	nBytes += (delay_line_max_samples(p, n) * (int32_t)sizeof(MYFLT));
	nBytes = (nBytes + 15) & (~15);
	return nBytes;
}

static void next_random_lineseg(SC_REVERB *p, reverbsc_delayLine *lp, int32_t n)
{
...
static void init_delay_line(SC_REVERB *p, reverbsc_delayLine *lp, int32_t n)
{
	double  readPos;
	/* int32_t     i; */
...
void sc_reverb_init(SC_REVERB *p)
{
	...
	if (nBytes != (int32_t)p->auxData.size)
		AuxAlloc((size_t)nBytes, &(p->auxData));
	else if (p->initDone && (p->iSkipInit) != FL(0.0))
		return;    /* skip initialisation if requested */
	      /* set up delay lines */
	nBytes = 0;
	for (i = 0; i < 8; i++) {
		p->delayLines[i] = (reverbsc_delayLine *)((unsigned char *)(p->auxData.auxp)
			+ (int32_t)nBytes);
...
void sc_reverb_perf(uint32_t nsmps, SC_REVERB *p)
{
	...
	for (i = 0; i < nsmps; i++) {
		...
		ainL *= jpScale;
		ainR = ainL + (double)p->ainR[i * p->kainRStride];
		ainL = ainL + (double)p->ainL[i * p->kainLStride];
		...
		p->aoutL[i * p->kaoutLStride] = (MYFLT)(aoutL * outputGain);
		p->aoutR[i * p->kaoutRStride] = (MYFLT)(aoutR * outputGain);
	}
}
